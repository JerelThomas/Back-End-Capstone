...
const { UnauthorizedError } = require("../helpers/expressErrors");
...
function ensureAdmin(req, res, next) {
  try {
    ...
    if (!res.locals.user || res.locals.user.IsAuth===false) {
      throw new UnauthorizedError();
    }
...
function ensureCorrectUserOrAdmin(req, res, next) {
  try {
    ...
    if (!(user && (user.IsAuth || user.id === parseInt(req.params.id)))) {
      throw new UnauthorizedError();
    }
    return next();
  } catch (err) {
